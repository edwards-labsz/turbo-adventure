...
Uploads one level with a random name: <word> <roman numeral>
Name format: e.g. "Cascade XLII", "Vortex VIII", "Ember DCCXLIX"
"""
...
WORDS = [
    ...
    "eclipse","echo","edge","epoch","fable","flare","flux","forge","fury","gale","glyph",
    ...
    "ozone","paragon","peak","petal","phantom","pillar","pinnacle","prism","pyre","quartz",
...
def to_roman(n: int) -> str:
    """Convert integer 0-1000 to Roman numeral (0 → 'N')."""
    if n == 0:
        return "N"
...
def random_level_name() -> str:
    ...
    return f"{word} {to_roman(number)}"

def generate_gjp2(password: str) -> str:
    """SHA-1( password + 'mI29fmAnxgTs' )"""
...
def generate_upload_seed(data: str, chars: int = 50) -> str:
    """Pick 50 evenly-spaced chars from the level string."""
    if len(data) < chars:
        return data
    step = len(data) // chars
